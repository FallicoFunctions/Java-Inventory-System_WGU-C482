...
/** This class is the modify part controller screen. It handles modifying parts in the inventory system.*/
public class ModifyPartController implements Initializable {
    ...
    public void onMouseSave(MouseEvent mouseEvent) throws IOException {
        ...
        String companyS = companyField.getText(); //Get company name or machineID
            ...
            try {
                ...
                if (errPartValid.length() > 0) { //Checks text field validation and give error messages if criteria not met
                    ...
                    if (this.outsourcedButton.isSelected()) { //If the Outsourced toggle button is selected a new Outsourced part is saved
                        Outsourced updatePart = new Outsourced(id,name,price,inv,min,max,companyS);
                        Inventory.updatePart(selectedPartIndex,updatePart);
                    }
                    else {
                        int machineId = 0;
                        try {
                        ...
                        InHouse updatePart = new InHouse(id,name,price,inv,min,max,machineId); //If the InHouse toggle button is selected a new InHouse part is saved
                        Inventory.updatePart(selectedPartIndex,updatePart);
                    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (selectedPart instanceof InHouse) {
            inOutSelector.selectToggle(inHouseButton);
            companyField.setText(Integer.toString(((InHouse) selectedPart).getMachineID()));
        }
        else {
            inOutSelector.selectToggle(outsourcedButton);
            companyField.setText(((Outsourced) selectedPart).getCompanyName());
            radioButtonLabel.setText("Company Name");
        }
    }
}
